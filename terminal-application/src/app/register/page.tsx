...
export default function RegisterPage() {
  ...
  const { loading, error, setError, send } = useAuthRequest();

  // Warm up the NextAuth API route on mount to eliminate cold-boot delay in dev mode.
  useEffect(() => {
    fetch("/api/auth/providers").catch(() => { });
  }, []);

  // Strong passkey rules — mirror these on the server later
  const rules = [
  ...
  const passkeyStrong = rules.every((r) => r.test(password));
  const mismatch = confirm.length > 0 && password !== confirm;

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!passkeyStrong) {
      setError("Weak passkey — satisfy all requirements");
      return;
    }
    if (password !== confirm) {
    ...
    const ok = await send(
      "/api/auth/signup",
      { email, password },
      "Enrollment rejected — User may already exist"
    );
    if (ok) router.push("/login"); // signup returns no session cookie, so sign in next
  };
      ...
      <div className="flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <p className="mt-2 font-mono text-[11px] uppercase tracking-[0.25em] text-muted">
            New operative enrollment / BitBash Sentry
          </p>
                ...
                <ul className="mt-3 space-y-1.5">
                  {rules.map((r) => {
                    ...
                    return (
                      <li
                        key={r.label}
                        className={`font-mono text-[10px] uppercase tracking-[0.15em] ${ok ? "text-primary" : "text-muted"
                          }`}
                      >
                        {ok ? "✓" : "○"} {r.label}
                      </li>
            ...
            <div>
              ...
              <div className="relative mt-2">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className={`w-full rounded-lg border bg-surface py-3 pl-11 pr-4 font-mono text-sm text-foreground placeholder:text-muted focus:outline-none ${mismatch
              ...
              {mismatch && (
                <p className="mt-2 font-mono text-[11px] uppercase tracking-[0.15em] text-red-500">
          ...
          <button
            ...
            className="flex w-full cursor-pointer items-center justify-center gap-3 rounded-lg border border-line bg-surface py-3.5 font-mono text-xs font-bold uppercase tracking-[0.2em] text-foreground transition hover:border-primary/40 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {googleLoading ? (
